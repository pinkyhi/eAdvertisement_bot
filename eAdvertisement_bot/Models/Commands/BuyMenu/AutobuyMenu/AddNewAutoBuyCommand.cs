        ...
        public async override Task Execute(Update update, TelegramBotClient botClient)
        {
            ...
            try
            {
                if (dbContext.Publications.Count(p => p.User_Id == update.CallbackQuery.From.Id) < 8)
                {
                    ...
                    DbEntities.Autobuy newAb = new DbEntities.Autobuy { User_Id = user.User_Id, Balance = 0, Interval = 0, State = 0, Name = "New autobuy", Min_Price = 0, Max_Cpm = 0, Max_Price = 0, Daily_Interval_From = new TimeSpan(0,0,0), Daily_Interval_To = new TimeSpan(0,0,0)  };
