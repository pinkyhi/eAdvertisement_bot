            ...
            try
            {
                ...
                if (channel.Places != null)
                {
                    foreach (Place p in places)
                    {
                        ...
                        List<Advertisement> nearestAds = dbContext.Advertisements.Where(a => a.Channel_Id == channelId && a.Date_Time <= new DateTime(tDT.Year, tDT.Month, tDT.Day, p.Time.Hours, p.Time.Minutes, p.Time.Seconds)).ToList();
                        Advertisement nearestAd = nearestAds.FirstOrDefault(a => a.Date_Time.Equals(nearestAds.Max(a => a.Date_Time)));

                        List<Advertisement> nearestTopAds = dbContext.Advertisements.Where(a => a.Channel_Id == channelId && a.Date_Time > new DateTime(tDT.Year, tDT.Month, tDT.Day, p.Time.Hours, p.Time.Minutes, p.Time.Seconds)).ToList();
